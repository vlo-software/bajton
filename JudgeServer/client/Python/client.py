...
rl.on('line', (input) => {
  ...
  console.log(a + b);
    ...
    ts_src = """const input = async () => {
  const buf = new Uint8Array(1024);
  const n = <number>await Deno.stdin.read(buf);
  return new TextDecoder().decode(buf.subarray(0, n)).trim();
};
const [a, b] = (await input()).split(" ").map(Number);
console.log(a + b);
"""
    ...
    print("ts_judge")
    print(client.judge(src=ts_src, language_config=ts_lang_config,
                       max_cpu_time=3000, max_memory=128 * 1024 * 1024,
                       test_case_id="normal", output=True), "\n\n")
