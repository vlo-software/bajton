...
<div ref="container" class="container">
    ...
    <div class="main">
      ...
      <form @submit="handleLogin" :model="formLogin" >
        <input v-model="formLogin.username" :placeholder="$t('m.LoginUsername')" type="text" @on-enter="handleLogin" />
        <input v-model="formLogin.password" :placeholder="$t('m.LoginPassword')" type="password" @on-enter="handleLogin" />
        <input v-if="tfaRequired" v-model="formLogin.tfa_code" :placeholder="$t('m.LoginTFA')" type="text" />
        <button v-if="!btnLoginLoading" type="submit" >{{$t('m.LoginBtn')}}</button>
...
<script>
  ...
  export default {
    ...
    watch: {
      'errors' () {
        if (this.errors.length === 0) {
          this.$refs.container.style.setProperty('--form-height', this.tfaRequired ? '580px' : '500px')
        } else {
          this.$refs.container.style.setProperty('--form-height', `${(this.tfaRequired ? 580 : 500) + 22 * this.errors.length}px`)
        }
      },
      'tfaRequired' () {
