...
<div ref="container" class="container">
    ...
    <div class="main">
      ...
      <form @submit="handleReset" :model="formReset" >
        <input v-model="formReset.password" :placeholder="$t('m.ResetPasswordPassword')" type="password" @on-enter="handleReset" />
        <div class="captcha">
          <input v-model="formReset.captcha" :placeholder="$t('m.ResetPasswordCaptcha')" type="text" @on-enter="handleReset" />
          <img alt="captcha" :src="captchaSrc" @click="getCaptchaSrc"/>
        </div>
        <button v-if="!btnResetLoading" type="submit" >{{$t('m.ResetPasswordBtn')}}</button>
...
<script>
  ...
  export default {
    ...
    mounted () {
      this.formReset.token = this.$route.params.token
      this.getCaptchaSrc()
      console.log(this.formReset.token)
    },
    data () {
      return {
    ...
    methods: {
      async handleReset (e) {
        ...
        if (this.formReset.password === '') {
          this.errors.push(this.$t('m.ResetPasswordErrorMissingPassword'))
        } else if (this.formReset.password.length < 6 || this.formReset.password.length > 20) {
          this.errors.push(this.$t('m.ResetPasswordErrorPasswordOutOfBounds'))
        }

        if (this.formReset.captcha === '') {
          this.errors.push(this.$t('m.ResetPasswordErrorMissingCaptcha'))
        } else if (this.formReset.captcha.length < 1 || this.formReset.captcha.length > 10) {
    ...
    watch: {
      'errors' () {
        if (this.errors.length === 0) {
          ...
          this.$refs.container.style.setProperty('--form-height', `${460 + 22 * this.errors.length}px`)
        }
      }
    }
  }
