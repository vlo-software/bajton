<template>
  <div class="setting-main">
    ...
    <div class="mini-container setting-content">
      <Form>
        <Alert v-if="TFAOpened" type="success" class="notice" showIcon
        ...
        <template v-if="!loadingQRcode">
          ...
          <Button type="error" :loading="loadingBtn" @click="closeTFA" v-else
            >Close TFA
          </Button>
        </template>
      </Form>
  ...
  methods: {
    ...
    deleteSession (sessionKey) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          api.deleteSession(sessionKey).then(
            (res) => {
              this.getSessions()
            },
            (_) => {}
    ...
    updateTFA (close) {
      ...
      api.twoFactorAuth(method, this.formTwoFactor).then(
        (res) => {
